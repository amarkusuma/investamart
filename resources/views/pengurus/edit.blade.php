...
<form action="{{ route('pengurus-komda.update', $pengurus->id) }}" method="POST">
    @csrf
            ...
            <div class="row">
                <div  class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="exampleFormControlSelect1" name="user_id" required>
                            <option selected value="{{$pengurus->user->id}}">{{$pengurus->user->name}}</option>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="exampleFormControlSelect1" name="komda_id" required>
                            ...
                            @foreach ($komda as $data)
                             <option value="{{$data->id}}" {{($pengurus->komda && $data->id == $pengurus->komda->id ) ? 'selected' : ''}}>{{$data->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        ...
                        <input type="text" name="jabatan" class="form-control" placeholder="jabatan" value="{{$pengurus->jabatan}}">
                    </div>
                </div>
